...
- require architecture, multi-role, dispatch, callback, and registry work to read this skill plus the project role-window ledger before creating or continuing role windows.
...
## Architecture Entry Guard Rule

For architecture, multi-role, dispatch, callback, or role-window registry tasks, the active window must use this skill before creating, continuing, dispatching, or retiring any role window. Do not rely on chat memory alone.

Required preflight:
- read `C:\Users\12156\.codex\skills\agent-role-orchestrator\SKILL.md` when running on this machine, or the equivalent installed `agent-role-orchestrator/SKILL.md` on another machine;
...
```text
架构/多角色/派发/回调/台账类任务必须先使用 agent-role-orchestrator。
执行前必须读取：
...
- architecture, multi-role, dispatch, callback, and registry tasks read `agent-role-orchestrator/SKILL.md` and the project `.codex/role-windows.md` before creating or continuing role windows.
...
- architecture and multi-role prompts include the `路由前检查` checklist.
