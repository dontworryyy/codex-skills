...
- `总控` / `CEO`: `gpt-5.6-terra` + `high`;资金、上线、生产恢复或跨角色最终 go/no-go 才升级 `gpt-5.6-sol` + `xhigh`.
- `架构` / `CTO`: `gpt-5.6-sol` + `high`;实盘架构、事故根因、DB/并发/安全或不可逆方案升级 `xhigh`；只有极难问题才用 `max`.
- `开发负责人` / `Dev Lead` (`开发` window): `gpt-5.6-terra` + `high`;它负责拆解、集成、纠偏和最终提交。live exit、资金安全、PnL/fee、并发或重复失败返工升级 `gpt-5.6-sol` + `xhigh`.
- `开发执行 subagent`: 单文件、测试明确、机械实现用 `gpt-5.4-mini` + `high`;两三个文件且需要理解业务语义用 `gpt-5.6-terra` + `high`. 它是窗口内一次性 subagent，不是持久角色窗口。
- `QA`: 普通验收用 `gpt-5.6-terra` + `high`;关键 PR、对抗式审查、发布门禁或生产风险用 `gpt-5.6-sol` + `xhigh`.
- `运维` / `DBA`: 只读采证、容量、锁和空间分析用 `gpt-5.6-terra` + `high`;真正部署、restart、rollback、生产故障、DDL、清理、恢复或数据风险用 `gpt-5.6-sol` + `xhigh`.
- `知识库`、`技能维护` and `文档/交付`: 默认 `gpt-5.6-terra` + `high`;策略结论、收益判断、知识结构或跨角色治理保持 Terra；已确认范围内的纯索引、排版、搬运或 registry 同步可降到 `gpt-5.4-mini` + `medium`.
- `UI/PPT` (including diagnostic-page design prototypes) and content roles: default to `gpt-5.6-terra` + `high`;机械 HTML/资产整理可用 `gpt-5.4-mini` + `high`; high-risk public positioning, claims, compliance, or cross-platform strategy uses `gpt-5.6-sol` + `xhigh`.

`gpt-5.3-codex-spark` remains a research preview and is not the default execution route. For long or compact-prone development tasks, do not make a low-cost subagent the long-running owner. Keep ownership in the `开发负责人` / `Dev Lead` window and delegate only bounded execution slices. These are in-window one-shot subagents, not reusable role windows: do not write them into `.codex/role-windows.md`, do not assign them persistent thread ids, and close them after the task ends. The Dev Lead must write the task card first: goal, allowed files, forbidden scope, validation command, expected output, and callback target. Subagents must not own architecture decisions, cross-file integration, correction strategy, final verification, or commits.
...
`内容主编` owns the `反老登味 / 反 AI 味内容闸门` for public-facing content. Before formal outward copy leaves the content domain, `内容主编`, `公众号发布`, `小红书`, `视频`, and `UI/PPT` public copy must check:

- 社交平台中文文案先按需用 `$social-text-websense-gate`：网感来自荒谬点识别，不来自机械堆网词；尤其要防 README 腔、技术发布会腔和 AI 方法论腔。
- 用户手改文案或指出“不像我 / 太编辑 / 没网感”时，用 `$content-style-calibration-loop` 提炼可复用风格规则，不要只记表层词。
- 反老登味：avoid 说教、爹味、上位者口吻、油腻成功学、年龄/资历压人、替读者下判断.
- 反 AI 味：avoid 模板化、空泛排比、万能套话、机械转折、过度总结、没有个人判断.
- Keep real experience, concrete details, natural spoken rhythm, platform voice, and the reader's actual situation.
- Do not change facts, data, prices, dates, source attribution, authorization boundaries, publishing status, endorsements, or effect claims.
- `$humanizer-zh` is still required for final formal Chinese copy; the tone gate defines taste and risk boundaries instead of replacing that skill.
...
- `UI/PPT` Xiaohongshu/Rednote new carousel visuals, cover redesigns, and full visual rewrites: `$xhs-visual-director`;
- `UI/PPT` small Xiaohongshu/Rednote social-card edits, legacy carousel packages, or WeChat cover pairs: `$guizang-social-card-skill`;
...
- `内容主编` content route planning across WeChat, Xiaohongshu, video, and visual assets: use `$content-model-handoff` when raw text is drafted by a separate model/window, then route to the same content role tools below and enforce approval gates;
- `公众号发布` WeChat Official Account AI application article operations, gzh-design handoff, draft-box updates, weekly continuity, manifest publish gates, and publishing handoff: `$wechat-ai-app-ops`;
...
- `小红书` Xiaohongshu/Rednote note packaging, carousel assets, captions, tags, content experiments, comment research, and publishing automation: use `$cheat-on-content` for topic scoring, blind prediction, benchmark learning, post-publish retro, and rubric evolution; use `$xhs-comment-research` for comment collection/analysis when the user asks to use comment data; use `$xhs-visual-director` for new carousel visuals, cover redesigns, full visual rewrites, style judgment, visual master, image prompts, confirmation-image-first flow, and anti-template visual gate; use `$humanizer-zh` for title/caption/body copy humanization before final packaging; use `$xhs-publish-assistant` for copy-ready publish bundles; use `$xhs-automation-publisher` for authorized browser automation, preview fill, publish blockers, content-data export, and approved posting; use `$guizang-social-card-skill` only for legacy/small social-card production when needed; then apply explicit user authorization gates before posting or interaction;
...
- `开发` is the `开发负责人` / `Dev Lead` by default: it uses `gpt-5.6-terra` + `high` to break down coding work, integrates results, runs final tests, and commits when asked or when workspace instructions require it. It may delegate mechanical single-file slices to `gpt-5.4-mini` + `high`, or bounded two-to-three-file semantic slices to `gpt-5.6-terra` + `high`; live/资金/并发/账本 and repeated-failure work stays with the Dev Lead on `gpt-5.6-sol` + `xhigh`. It should not own UI/visual direction when the dominant risk is visual fidelity.
...
- `内容主编` owns content-domain routing under `总控`: `公众号发布`, `小红书`, `视频`, and `UI/PPT` visual-asset collaboration; it enforces fact discipline, account boundaries, content-model handoff, public-writing gates, and explicit publish approvals.
- `公众号发布` uses `$wechat-ai-app-ops`, runs `$humanizer-zh` before final preview/draft handoff, prefers `gzh-design` for formal article layout when available, prepares and automates WeChat Official Account article drafts/previews by default, reads manifest gates before publish, and requires explicit approval before final publish.
- `小红书` may use `$cheat-on-content` for social-content scoring, blind prediction, benchmark learning, and retro loops; it uses `$xhs-visual-director` for new carousel visuals, cover redesigns, full visual rewrites, confirmation images, and anti-template visual review, uses `$humanizer-zh` before final note/publish copy, uses `$xhs-publish-assistant` for copy-ready publish bundles, uses `$xhs-automation-publisher` for authorized browser automation with preview/fill defaults, and requires explicit approval before final posting or interaction.
