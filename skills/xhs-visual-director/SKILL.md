...
## Anti-template visual gate

小红书 AI / Codex / Skill / workflow 题材不要默认做成 SaaS 发布会、暗色科技模板或圆角卡片 PPT。视觉方向必须先服务内容的前三秒理解和账号真实感，再考虑设计炫技。

默认检查：

- 不要默认暗色科技、蓝紫霓虹、玻璃拟态、巨大页码、胶囊标签、大白圆角卡片。
- 不要把所有 Agent / Codex / Skill 选题都做成“高端 AI 产品发布会”。如果内容是被项目折磨、真实工作流、仓库实验，可以更像产品杂志、工作台、代码现场、截图裁切、资料夹、纸面批注或真实工具界面。
- 真实截图、产品界面、GitHub 页面、后台数据、原帖截图可以作为事实锚点，但不能硬贴。截图用于证明或解释，不是为了填满版面。
- 可以增强真实素材的视觉呈现（裁切、遮罩、相框、旁注、局部放大），但不要改写事实、伪装来源或把截图做成自制数据图。
- 图卡优先让读者 1 秒看懂“这篇和我有什么关系”，而不是展示设计系统有多完整。
- 如果用户已经表达审美偏好，优先遵守用户偏好；不要用本 skill 默认风格覆盖它。

通过标准：用户能看出这套图文属于一个真人账号的内容资产，而不是模板网站批量导出的 AI 科技卡。
...
同时判断信息是否足够直接生成。如果用户只给了一个很短的主题、模糊目标、缺少受众、缺少发布目的、缺少素材说明、缺少风格偏好或缺少页数要求，不要直接生成完整方案，先进入 Step 1.5。

### Step 1.5：苏格拉底式提问澄清

当信息不足时，先向用户提出 10-20 个苏格拉底式问题，再生成图文方案。提问目标不是收集表面偏好，而是帮助用户想清楚：
...
- 用户已经明确给出主题、目标受众、页数、传播目标、风格偏好、素材和输出格式。
...
- 默认一次性提出 10 个问题。
- 复杂商业、产品、课程、个人品牌或多素材项目，可以提出 15-20 个问题。
...
我先不直接生成，因为现在的信息还不足以稳定判断风格和页面结构。请尽量逐条回答；如果某题不确定，可以写“你来判断”。
...
- 必须通过 Anti-template visual gate：不要默认暗色科技模板、SaaS 发布会感、大白圆角卡、胶囊标签或硬贴截图；如果使用真实素材，说明它承担的事实锚点或理解降低作用。
...
- 是否有收藏价值
- 是否避免了模板感
- 是否通过 Anti-template visual gate（没有默认暗色科技模板 / SaaS 发布会感 / 大白圆角卡 / 胶囊标签 / 硬贴截图）
- 使用真实截图或产品界面时，是否说明其事实锚点作用，且没有伪装来源
- 是否适合小红书滑动阅读
- 是否太像 PPT
- 是否太像廉价 AI 模板
- 封面和内页是否有节奏变化
...
如果信息不足，先输出“先做 10 个澄清问题”，等待用户回答后再继续。
