...
name: story-deslop
version: 1.0.0
description: Use when editing Chinese web fiction, narrative prose, or story drafts that sound AI-generated, overly polished, formulaic, explanatory, or too symmetrical; use to detect AI-writing fingerprints and revise only wording while preserving plot, facts, characters, names, and intent.
metadata:
  short-description: Chinese narrative anti-AI-flavor rewriting
  source: https://github.com/worldwonderer/oh-story-claudecode/tree/main/skills/story-deslop
  upstream_ref: ef8ae1e30fb01aee7ea3bc451d9e3e92a9287b9b
  extraction: story-deslop only; excludes story setup, scan, analyze, write, cover, agents, hooks, and browser skills
---
...
这是从 `worldwonderer/oh-story-claudecode` 中单独抽取的 `story-deslop` 去 AI 味写作 skill。只使用本目录内的 `SKILL.md`、`references/` 和 `scripts/`。

不要启动、安装、引用或要求以下上游组件：`story-setup`、`story`、`story-long-*`、`story-short-*`、`story-import`、`story-review`、`story-cover`、`browser-cdp`、`.claude/agents/*`、hooks、rules 或完整网文项目脚手架。

如果上游文本提到子代理或其他 story skill，按当前 Codex 会话内联执行本 skill 的检测和改写流程；不要 spawn `narrative-writer`，也不要把任务转交给其他 oh-story skill。

与 `humanizer-zh` 的边界：`humanizer-zh` 用于公众号、小红书、通用中文文案的人味化；`story-deslop` 只在文本是小说、叙事正文、剧情片段、对话场景或网文风格草稿时使用，避免把普通营销文案改成小说腔。
...
Phase 2 诊断完成后，在当前 Codex 会话内联执行 Gate A-F。不要依赖 `.claude/agents/narrative-writer.md`，不要 spawn 子代理，也不要调用 oh-story 的其他 skill。
...
- 只负责检测和改写叙事文本的 AI 味。
- 发现结构问题、剧情逻辑问题、设定冲突或封面需求时，只在报告中标注，不自动跳转到其他 story skill。
- 用户要求继续创作、拆文、封面、扫榜或完整网文项目管理时，说明当前抽取版不包含这些能力，再按可用本地角色或其他 skill 重新判断。
