...
| `UI/PPT` / `UI/Frontend` | Codex 本地窗口 | UI 体验、视觉改造、前端视觉保真、预览图实现路线选择、网页 PPT、社交卡、公众号封面、小红书图文视觉、演示材料、照片到玩具资产规划 | `gstack-design-*`, `design-taste-frontend`, `guizang-ppt-skill`, `xhs-visual-director`, `guizang-social-card-skill`, `photo-to-cute-3d-toy`, `playwright` |
...
| `小红书` | Codex 本地窗口 | 小红书/Rednote 笔记、评论研究、图文视觉、标题标签、内容实验、发布复制包和授权发布自动化 | `cheat-on-content`, `xhs-comment-research`, `xhs-visual-director`, `humanizer-zh`, `xhs-publish-assistant`, `xhs-automation-publisher`, `guizang-social-card-skill`, `playwright`；最终发布必须明确授权 |
...
| `xhs-automation-publisher` | 小红书 / 内容主编 | 小红书登录检查、创作者中心预览填充、自动发布卡点排查、内容数据导出、搜索/详情读取和明确授权后的最终发布；默认 `--preview` / `fill`，点击发布和互动动作二次授权 |
...
- `小红书` 需要登录检查、浏览器预览填充、发布卡点排查、搜索/详情读取、内容数据导出或授权发布时使用 `xhs-automation-publisher`；默认先用 `--preview` 或 `cdp_publish.py fill`，因为 `publish_pipeline.py` 缺少 `--preview` 会点击发布。
