...
3. 复杂新需求先输出 3-5 个可行技术路线的选型简报，再进入下游实施。
4. 新本地代码项目默认先检查或初始化 CodeGraph；未安装时提示安装，或在环境允许且安装方式明确时做用户级静默安装。
5. 需求确认到足以描述问题后，`架构` 先做有边界的开源/可借鉴方案扫描；若网络不可用、用户禁用或上下文敏感，要写明跳过原因。
6. `架构` 只在必要时输出 `开发`、`UI/PPT` / `UI/Frontend`、`视频`、`公众号发布`、`小红书`、`文档/交付`、`知识库`、`技能维护`、`DBA`、`测试`、`QA`、`安全`、`运维` 等下游提示词。
7. 已建立过的角色默认走 `继承` / `接续`，不要重复新建窗口。
8. 下游角色完成、阻塞或需要发起方决策时，用 `压缩回调` 默认回调任务发起窗口；不无条件回报给 `架构`。
9. 只有明确需要并行时才开 `开发1号`、`开发2号` 这类编号角色。
