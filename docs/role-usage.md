...
这份文档说明本仓库的结构、角色边界和推荐运行环境。核心原则是：`架构` 先判断需求、为新本地代码项目启动 CodeGraph、扫描开源/可借鉴方案并拆分边界，复杂需求先给多方案技术选型，`开发` 等执行角色按白名单落地，`公众号发布` / `小红书` / `文档/交付` / `知识库` / `技能维护` / `DBA` 可作为独立角色单独唤起，`运维` 优先交给服务器侧 Hermes agent 做只读诊断和受控操作。
...
| --- | --- | --- | --- |
| `架构` | Codex 本地窗口 | 需求澄清、多方案技术选型、新项目 CodeGraph 启动、开源/可借鉴方案扫描、架构判断、拆分角色、维护角色台账 | `agent-role-orchestrator`, `gstack`, `gstack-office-hours`, `gstack-spec`, `gstack-autoplan`, `gstack-plan-*`, `startup-pressure-test` |
...
| `xhs-comment-research` | 小红书 / 架构 | 小红书评论读取、分类、总结和受众/选题研究，必须保持浏览器登录态边界 |
...
| `gstack-document-*`, `gstack-learn`, `gstack-retro`, `delivery-document-package` | 架构 / 开发 / QA / 文档/交付 | 文档、发布说明、交付材料、经验沉淀、复盘 |
...
3. 架构/多角色/派发/回调/台账类任务必须先读取已安装的 `agent-role-orchestrator/SKILL.md` 和项目 `.codex/role-windows.md`；若台账缺失或不可读，状态写 `待确认`，不要编造线程 ID。
4. 复杂新需求先输出 3-5 个可行技术路线的选型简报，再进入下游实施。
5. 新本地代码项目默认先检查或初始化 CodeGraph；未安装时提示安装，或在环境允许且安装方式明确时做用户级静默安装。
6. 需求确认到足以描述问题后，`架构` 先做有边界的开源/可借鉴方案扫描；若网络不可用、用户禁用或上下文敏感，要写明跳过原因。
7. `架构` 只在必要时输出 `开发`、`UI/PPT` / `UI/Frontend`、`视频`、`公众号发布`、`小红书`、`文档/交付`、`知识库`、`技能维护`、`DBA`、`测试`、`QA`、`安全`、`运维` 等下游提示词。
8. 已建立过的角色默认走 `继承` / `接续`，不要重复新建窗口；`.codex/role-windows.md` 中已有线程 ID 时必须复用。
9. 下游角色完成、阻塞或需要发起方决策时，用 `压缩回调` 默认回调任务发起窗口；不无条件回报给 `架构`。
10. 每次派发、回调、阻塞、完成、误开或纠偏后，更新 `.codex/role-windows.md` 的最新结果、下一步和循环状态。
11. 只有明确需要并行时才开 `开发1号`、`开发2号` 这类编号角色。
...
- `架构` 为多角色任务维护轻量 `技能路由台账`：候选 skill、必选 skill、可选 skill、跳过 skill 及原因、预期加载角色。
- 下游角色完成、阻塞或需要决策时，用 `技能命中回传` 说明已加载并使用、架构要求但未使用、临时发现应补用、误召/无效加载和真正影响产出的 skill。
- `架构` 验收时必须检查 `技能命中回传`；缺失时退回补 callback，不算闭环完成。
- 多窗口 loop 默认用 `压缩回调`，只传当前状态、本轮变化、证据链接/文件/命令、需要决策、下一回流对象和可复用优化沉淀状态。
- `架构` 负责本次任务的路由判断和聚合视图；长期的漏召、误召、触发描述过期、registry 漂移、README 说明混乱或跨角色 token 过重，转给 `技能维护`。
...
- 不替代 `架构` 做本次任务决策，不替代 `开发` 实现产品，不把项目私有状态同步进公开仓库。
...
如果一个任务同时需要测试报告、压测证据和 Review readiness，先让 `架构` 拆成 `测试` 与 `QA` 两个提示词，避免职责混在一起。
...
- 新需求先过 `架构`。
- 复杂新需求先看多条技术路线，选定路线再拆下游。
- 新本地代码项目先检查或初始化 CodeGraph，未安装则提示安装或在可行时静默安装。
- `架构` 确认需求后，先做有边界的开源/可借鉴方案扫描，再决定是否拆给下游角色。
- 代码和产物交给 Codex。
- 远程生产事实交给 Hermes 只读查。
