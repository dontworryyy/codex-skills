...
- 下游完成、阻塞或需要决策时，用压缩回调通知任务发起窗口，不默认都回总控或架构。
...
上下文预算规则：当窗口历史变长时，不等 remote compact 失败后再补救。`.codex/role-windows.md` 需要保留固定角色表格和 `压缩交接卡`，用最近摘要、关键决策、证据句柄、下一步和新窗口接续提示支撑续跑。回调只传增量、证据和决策需求，不搬运完整聊天记录。
