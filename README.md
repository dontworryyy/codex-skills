...
`xhs-automation-publisher` 引入自 [white0dew/XiaohongshuSkills](https://github.com/white0dew/XiaohongshuSkills)，用于小红书登录检查、创作者中心预览填充、自动发布卡点排查、内容数据导出、搜索/详情读取，以及明确授权后的最终发布。

- 默认优先 `--preview` 或 `cdp_publish.py fill`，先让用户人工复核。
- `publish_pipeline.py` 不带 `--preview` 会点击发布，必须视为高风险写操作。
- `click-publish`、评论/回复、点赞收藏、切号、清 cookie、删除 profile 等动作必须二次明确授权。
- cookie、二维码、真实账号名、Chrome profile 路径和账号状态不能写入仓库或回调正文。
...
入口 skill 也采用按需加载：`SKILL.md` 只保留稳定闭环契约；角色边界、模型分级、工具路由和内容平台规则分别放在 `references/role-cards.md`、`model-routing.md`、`tool-routing.md`、`content-routing.md`。一次技术执行不再顺带加载所有内容平台、DBA、运维和发布细则，PR 校验同时限制入口文件体积，防止后续再次膨胀。
...
| `小红书` | 小红书笔记、图文视觉、标题标签、发布复制包、授权发布自动化 | 新图文视觉默认先走视觉导演；浏览器自动化默认预览/填充，最终发布和互动需二次授权 |
